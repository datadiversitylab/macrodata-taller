...
# Nombre 1: [Melany Andrea Consuegra]
# Nombre 2: [Juan Camilo Diaz]
...
equipo <- "barranquilla-equipo08" # Tienes que modificar la ciudad y el numero
salida <- file.path("equipo08", equipo)

datos <- read.csv("datos/birdbase/birdbase.csv", check.names = FALSE, stringsAsFactors = FALSE)
colibries <- datos[datos$`Famil IOC 15.1` == "Trochilidae", ]
View(colibries)
...
masa_max_total = datos$Xmax
masa_min_total =datos$Xmin

# --- DECISION 4. Especies con L, F o M ---------------------------------------
# Descartar, convertir al punto medio, imputar, tratar como categorica

convertir_altitud <- function(masa) {
  datos <- trimws(as.character(datos))
  equivalencias <- c(L = 250, F = 750, M = 1500)
  numerico <- suppressWarnings(as.numeric(datos))
  letra <- datos %in% names(equivalencias)
  numerico[letra] <- equivalencias[datos[letra]]
  numerico
}
